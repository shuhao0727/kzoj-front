...
// 示例讨论数据
const discussions = [
  { id: 1, title: '退役杂记', author: 'henpolorAC', role: 'ADM', comments: 10, likes: 1, views: 79, category: '寄语', date: '1个月前', isTop: true },
  { id: 2, title: '关于“什么是一篇好的题解”', author: 'henpolorAC', role: 'ADM', comments: 4, likes: 0, views: 89, category: '闲聊', date: '6个月前', isTop: false },
  // 其他讨论数据...
];
...
const Discuss = () => {
  ...
  const [categories, setCategories] = useState(['闲聊', '题解', '求助', '建议', '笔记', '寄语']);
  ...
  return (
    <div className="container mx-auto p-6">
      <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0 mb-4">
        <h1 className="text-2xl font-semibold text-gray-800">全部讨论 ({discussions.length})</h1>
        <input
          type="text"
          value={searchTerm}
      ...
      <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-6">
                ...
                <li key={index} className="flex justify-between items-center">
                  {isEditing ? (
                    ...
                    <button
                      ...
                      className={`block text-left w-full px-3 py-1 text-sm rounded-lg transition ${categoryFilter === category ? 'bg-blue-500 text-white' : 'bg-gray-100 hover:bg-gray-200'}`}
                    >
                      {category}
                    </button>
                  )}
        ...
        <div className="md:w-4/5 space-y-6">
          {discussions
            .filter(discussion => discussion.title.includes(searchTerm) || discussion.category === categoryFilter || categoryFilter === '全部')
            .map((discussion) => (
              <div key={discussion.id} className="border rounded-lg shadow-md p-4 bg-white hover:bg-gray-50 transition duration-300">
                <div className="flex justify-between items-center mb-2">
                  <div className="flex items-center space-x-2">
                    {discussion.isTop && (
                      <span className="bg-red-500 text-white px-2 py-1 rounded-lg text-xs font-bold">TOP</span>
                    )}
                    <h3 className="text-md font-semibold">{discussion.title}</h3>
                  </div>
