          ...
          {paginatedProblems.map((problem) => (
            <tr key={problem.id} className="border-b">
              ...
              <td className="p-2 text-left">
                {/* 使用动态路由 */}
                <Link href={`/problem/problemDetails/${problem.id}`}>
              ...
              <td className="p-2 text-right">
                <span className={`px-2 py-1 rounded text-white text-xs ${problem.difficulty === "入门" ? "bg-red-400" : problem.difficulty === "普及-" ? "bg-orange-400" : "bg-green-400"}`}>
                  {problem.difficulty}
                </span>
              </td>
              <td className="p-2 text-right">
                <div className="relative w-full bg-gray-200 rounded-full h-4">
                  <div className="bg-blue-500 h-4 rounded-full" style={{ width: `${problem.passRate * 100}%` }}></div>
                </div>
              </td>
            </tr>
          ))}
